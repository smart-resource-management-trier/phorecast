...
Broader energy and time-series software addresses related problems at different levels of abstraction. OpenSTEF provides open tooling for short-term energy forecasting pipelines [@OpenSTEF], while general-purpose libraries such as Darts provide unified APIs for a broad range of classical and machine-learning forecasting models [@Herzen2022Darts]. More recently, pretrained time-series forecasting models such as Chronos have explored foundation-model-style forecasting across diverse time-series domains [@Ansari2024Chronos]. PHORECAST is not intended to replace such domain-general forecasting libraries or pretrained forecasting models. Instead, it focuses on the PV forecasting workflow around the model: data loaders, orchestration, persistence, web/API access, Docker deployment, and reusable PV-focused preprocessing and LSTM training/inference functionality. The separation between `phorecast` and `phorecast-ml` is central to this contribution because it lets the same forecasting core support lightweight research use and full framework deployments.
